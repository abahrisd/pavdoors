var WebMarketVars={currencyBefore:true,currencySymbol:"$",priceRange:[0,1750],priceStep:50};jQuery(document).ready(function($){"use strict";if(/*@cc_on!@*/false){document.documentElement.className+=' ie10';}
...
if(selectedElms.length<1){filterString=".isotope-container .isotope--target";}else{var filterArr=[];selectedElms.each(function(){var data=$(this).data("target");if("undefined"!==typeof data){filterArr.push($(this).data("target"));}});filterString=filterArr.join(",");}
...
$container.isotope({filter:filter});};updateIsotopeFiltering();$("#isotopeSorting").change(function(){var parameters=jQuery.parseJSON($(this).val());parameters.sortAscending="true"===parameters.sortAscending?true:false;$container.isotope(parameters);});$("#isotopeSorting").trigger("change");});})();(function(){var tour=new Tour({useLocalStorage:true,backdrop:false});tour.addSteps([{element:"#tourStep1",title:"Filtering",content:"Filtering the products in Webmarket is fun!"},{element:"#tourStep2",title:"Categories",content:"Click to multiple categories and the articles on the right will be shown or hidden automatically."},{element:"#filterPrices",title:"Price Range Filter",content:"Select the price range, the products on the left will be filtered automatically, without page refresh!"},{element:"#tourStep3",title:"Open Additional Filters",content:"Just open the panel with the click on the title and you can start filtering the products even further... <br />The same way you can turn off the filter, just close the pane.",placement:"top"},{element:"#tourStep4",title:"The Best Part: Sorting!",content:"Never refresh a page again when you decide to order the items below. Just select ordering from the dropdown, the products below will magically fit into desired order",placement:"bottom"}]);tour.start();})();
	$(".add-googlemap").each(function(){
		...
		if(jQuery.goMap){
			var markersArr = [],
				...
				marker = {title:"undefined"===typeof $this.data("title")?false:$this.data("title")};

			addresses.forEach(function(el){
				if ("undefined" !== $this.data(el)) {
					marker.address = $this.data(el);
			...
			$this.goMap({
				markers:[{
					address:"ул. Красноармейская 1-я, 6, Пермь",
					icon: 'http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|b2e551',
					//title:"ул. Красноармейская 1-я, 6",
					html: {
						content: 'ул. Красноармейская 1-я, 6',
						popup: false
					}
				},{
					address:"ул. Героев Хасана, 56, Пермь",
					icon: 'http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|ee3e87',
					//title:"ул. Героев Хасана, 56"
					html: {
						content: 'ул. Героев Хасана, 56',
						popup: false
					}
				},{
					address:"ул. Уральская 63 к3, Пермь",
					icon: 'http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|77e0fa',
					//title:"ул. Уральская 63 к3"
					html: {
						content: 'ул. Уральская 63 к3',
						popup: false
					}
				}],
				scrollwheel:false,
	...
	$(".item-in-cart .icon-remove-sign").click(function(){$(this).parents(".item-in-cart").animate({opacity:0},"swing",function(){$(this).slideUp();});return false;});$(".table-items .icon-remove-sign").click(function(){var elmToRemove=$(this).parents('tr');if(!!$(this).data('delete-next')){elmToRemove=elmToRemove.add(elmToRemove.next());}
